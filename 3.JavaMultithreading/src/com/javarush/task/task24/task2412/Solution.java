    ...
    public static void sort(List<Stock> list) {
        list.sort(new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                int first = stock1.get("name").toString().compareTo(stock2.get("name").toString());
                if (first != 0) return first;
                else {
                    int second = stock1.get("date").toString().compareTo(stock2.get("date").toString());
                    if (second != 0) return second;
                    else {
                        double cost1 = (double) stock1.get("last") - (double) stock1.get("open");
                        double cost2 = (double) stock2.get("last") - (double) stock2.get("open");
